...
describe("Basic Result Utilities", () => {
  ...
  test("map2 - Ok", () => {
    let actual = Result.map2((x, y) => x + y, Result.return(1), Result.return(2))
    let expected = Result.return(3)
    Expect.expect(actual) |> Expect.toEqual(expected)
  })
  test("map2 - Error", () => {
    let actual = Result.map2((x, y) => x + y, Result.error(1), Result.error(2))
    let expected = Result.error(1)
    Expect.expect(actual) |> Expect.toEqual(expected)
  })
  test("map3 - Ok", () => {
    let actual = Result.map3(
      (x, y, z) => x + y + z,
      Result.return(1),
      Result.return(2),
      Result.return(3),
...
describe("Result.Promise based utilities", () => {
  ...
  testPromise("ap - Ok", () => {
    ...
    Result.Promise.ap(value, fn) |> Js.Promise.then_(actual => {
      let expected = Result.return(42)
      Expect.expect(actual) |> Expect.toEqual(expected) |> Js.Promise.resolve
    })
  })
  testPromise("ap - Error", () => {
    ...
    Result.Promise.ap(value, error) |> Js.Promise.then_(actual => {
